...
def generate_energy_intervals(
    ...
    if fixed_intervals is not None:
        ...
        if fixed_intervals > horizon:
            raise ValueError(
                f"fixed_intervals={fixed_intervals} cannot exceed horizon={horizon}."
            )
...
def run_random_experiment(
    ...
    if fixed_jobs is None:
        ...
        print(f"Jobs per instance: {fixed_jobs} (fixed)")
        print(
            f"Horizon: {horizon_per_job} * {fixed_jobs} = "
            f"{horizon_per_job * fixed_jobs}"
        )

    if fixed_intervals is None:
        print("Energy intervals: random default range")
    else:
        print(f"Energy intervals: {fixed_intervals} (fixed)")
    print(
        "Green share: "
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Use exactly this many jobs in every generated instance. "
            "When set, the horizon is automatically jobs * horizon-per-job."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=6,
        help=(
            "Scheduling horizon per job when --jobs is used. "
            "Default: 6, so horizon = 6 * jobs."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Use exactly this many energy intervals in every instance.",
    )
...
if __name__ == "__main__":
    ...
    if args.jobs is not None and args.jobs <= 0:
        raise ValueError("Require --jobs > 0.")

    if args.horizon_per_job <= 0:
        raise ValueError("Require --horizon-per-job > 0.")

    if args.intervals is not None and args.intervals < 3:
        raise ValueError("Require --intervals >= 3.")

    if (
        args.jobs is not None
        and args.intervals is not None
        and args.intervals > args.horizon_per_job * args.jobs
    ):
        raise ValueError(
            "--intervals cannot exceed horizon = jobs * horizon-per-job."
        )
    ...
    if args.jobs is None and args.max_jobs > args.min_horizon:
        print(
