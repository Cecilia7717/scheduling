...
def max_flow_passes_schedule_gpu(
    ...
    *,
    ...
    Timing
    ------
    gpu_maxflow_seconds measures the complete Max Flow + Passes
    scheduling algorithm, in a way intended to be comparable with the
    CPU implementation.

    It INCLUDES:
        - input validation performed during interval splitting
        - splitting intervals at job boundaries
        - computing total processing
        - building the scheduling network

        - preparing pass 1
        - GPU max-flow computation for pass 1
        - processing/recovering pass-1 flow
        - building the pass-1 snapshot/schedule rows

        - setting pass-2 capacities
        - building the pass-2 residual network
        - GPU max-flow computation for pass 2
        - applying the pass-2 residual augmentation
        - processing/recovering pass-2 flow
        - building the pass-2 snapshot/schedule rows

        - setting pass-3 capacities, when needed
        - building the pass-3 residual network
        - GPU max-flow computation for pass 3
        - applying the pass-3 residual augmentation
        - processing/recovering pass-3 flow
        - building the pass-3 snapshot/schedule rows

        - feasibility checking
        - final schedule/timeline construction
        - final normalized-cost computation

    It EXCLUDES ECL integration overhead:
        - temporary work-directory setup
        - DIMACS file writing
        - DIMACS -> EGR conversion
        - converter subprocess overhead
        - ECL subprocess/interface overhead outside ECL's reported runtime
        - reading the ECL flow CSV
        - deleting temporary DIMACS/EGR/CSV files

    wall_seconds still measures the complete end-to-end runtime, including
    the excluded integration overhead above.
    ...
    Therefore approximately:
        ...
        gpu_maxflow_seconds
            +
        ecl_integration_overhead_seconds
    """

    # ========================================================
    # TRUE END-TO-END WALL TIMER
    # ========================================================

    wall_start = time.perf_counter()

    # ========================================================
    # Track overhead that should NOT count as algorithm time.
    #
    # We will eventually compute:
    #
    # algorithm time
    #   = wall time
    #   - ECL integration overhead
    #   - work-directory/setup overhead
    #
    # This lets preparation, residual-network construction,
    # snapshots, final schedule creation, etc. remain included.
    # ========================================================

    integration_overhead_seconds = 0.0

    # ========================================================
    # ALGORITHM WORK: INCLUDED
    # ========================================================
    ...
    # ========================================================
    # ECL/work-directory setup: NOT INCLUDED in algorithm time
    # ========================================================
    ...
    integration_overhead_seconds += (
        time.perf_counter() - setup_start
    )

    # ========================================================
    # Wrap run_ecl_maxflow so that we can measure how much
    # wall time it consumes beyond ECL's reported GPU runtime.
    #
    # Example:
    #
    # run_ecl_maxflow wall time = 20 ms
    # ECL reported GPU runtime  =  4 ms
    #
    # integration overhead      = 16 ms
    #
    # The 4 ms remains part of gpu_maxflow_seconds.
    # The 16 ms is removed.
    # ========================================================

    original_run_ecl_maxflow = globals()["run_ecl_maxflow"]

    def timed_run_ecl_maxflow(*args, **kwargs):
        nonlocal integration_overhead_seconds

        call_start = time.perf_counter()

        ecl_result = original_run_ecl_maxflow(
            *args,
            **kwargs,
        )
        ...
        reported_gpu_seconds = ecl_result.get(
            "gpu_seconds"
        )

        if reported_gpu_seconds is None:
            # If ECL does not report a GPU runtime, we cannot
            # safely separate GPU computation from integration
            # overhead. Treat the whole wrapper call as overhead.
            integration_overhead_seconds += (
                call_wall_seconds
            )
        else:
            # Everything in run_ecl_maxflow except the
            # ECL-reported GPU runtime is treated as interface/
            # integration overhead.
            overhead = (
                call_wall_seconds
                - reported_gpu_seconds
            )

            # Protect against tiny negative values caused by
            # timing precision / rounding.
            integration_overhead_seconds += max(
                0.0,
                overhead,
            )
    ...
    # The existing _run_initial_pass() and
    # _run_augmentation_pass() resolve run_ecl_maxflow from
    # this module's globals, so temporarily replace it.
    globals()["run_ecl_maxflow"] = timed_run_ecl_maxflow
    ...
    try:
        ...
        #   preparing arcs
        #   GPU max flow
        #   copying flow back to original edges
        #   validation
        #   recover_result
        #   schedule-row construction
        #
        # EXCLUDED automatically by wrapper:
        #   DIMACS/EGR/subprocess/CSV overhead
        # ====================================================
        ...
        #
        # INCLUDED:
        #   set pass-2 capacities
        #   construct residual graph
        #   GPU max flow
        #   apply residual augmentation
        #   recover/process result
        #   schedule rows
        # ====================================================
        ...
        # PASS 3
        #
        # Only execute if pass 2 did not finish all jobs.
        # ====================================================
        ...
        # ====================================================
        # FINAL SCHEDULE PROCESSING: INCLUDED
        # ====================================================

        feasible = (
            final["flow"] == total_processing
        )
        ...
        normalized_cost = schedule_cost(
            final
        )

        # ====================================================
        # Preserve raw ECL GPU time as a separate diagnostic.
        #
        # This is useful because:
        #
        # ecl_gpu_seconds
        #     = actual ECL-reported max-flow runtimes only
        #
        # gpu_maxflow_seconds
        #     = complete Max Flow + Passes algorithm runtime
        #
        # wall_seconds
        #     = complete end-to-end implementation runtime
        # ====================================================
        ...
        # ====================================================
        # Stop timing after all scheduling work is finished.
        # ====================================================
        ...
        # Complete scheduling algorithm time:
        #
        # everything above
        # minus external ECL/file/subprocess integration.
        gpu_maxflow_seconds = max(
            0.0,
            wall_seconds
            - integration_overhead_seconds,
        )
        ...
        result = {
            ...
            # -----------------------------------------------
            # MAIN timing value for CPU-vs-GPU comparison.
            #
            # Includes:
            # scheduling preparation + passes + processing +
            # GPU computation + final schedule.
            #
            # Excludes:
            # DIMACS/EGR/subprocess/CSV interface overhead.
            # -----------------------------------------------
            "gpu_maxflow_seconds": (
                gpu_maxflow_seconds
            ),
            ...
            # -----------------------------------------------
            # Pure ECL-reported max-flow computation only.
            # Useful for GPU scaling analysis.
            # -----------------------------------------------
            "ecl_gpu_seconds": (
                ecl_gpu_seconds
            ),
            ...
            # -----------------------------------------------
            # External/interface overhead removed from the
            # algorithm timing.
            # -----------------------------------------------
            "integration_overhead_seconds": (
                integration_overhead_seconds
            ...
            # -----------------------------------------------
            # Full real-world end-to-end time.
            # -----------------------------------------------
            "wall_seconds": (
                wall_seconds
            ),
        ...
        # Restore the original function even if an exception
        # happens during one of the passes.
        globals()["run_ecl_maxflow"] = (
            original_run_ecl_maxflow
        )
